...
def add_vote(vote: schemas.Vote, db: Session = Depends(get_db), current_user: int = Depends(oauth2.get_current_user)):
    ...
    post = db.query(models.Post).filter(models.Post.id == vote.posts_id)
    if not post:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                            detail=f"post doesn't exist")

    votes = db.query(models.Vote).filter(models.Vote.posts_id ==
                                         vote.posts_id, models.Vote.user_id == current_user.id)
    ...
    if vote.dir == 1:
        ...
        new_vote = models.Vote(posts_id=vote.posts_id,
